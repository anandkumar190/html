<?php
 ...
 if(isset($_GET['edit']))
  ...
  {	
	 ...
	 $res=mysqli_query($con,"select e.id,e.name,e.address, round( ( 6371 * acos( least(1.0,  cos( radians($lat) ) * cos( radians(latitude) ) * cos( radians(longitude) - radians($lng) ) + sin( radians($lat) ) * sin( radians(latitude) ) ) ) ), 3) as distance from employees e where e.usertype='3' order by distance asc");
	 $response=array();
